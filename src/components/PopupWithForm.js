...
function PopupWithForm({name, title, saveBtnText, isOpen, onClose, onSubmit, isValid, ...props}) {
    ...
    <div className={isOpen ? `popup popup_type_${name} popup_opened` : `popup popup_type_${name}`}
        ...
        <div className={`popup__container popup__container_type_${name}`}>
          ...
          <form className={`popup__form popup__form_type_${name}`} name={name} action="#"
            ...
            <button className={`popup__save-btn ${!isValid ? `popup__save-btn_disabled` : ``}`} type="submit"
                    disabled={!isValid}>{saveBtnText}</button>
          </form>
        </div>
      </div>
  )
